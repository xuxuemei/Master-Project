...
VAT = 0.25
Spot_price = price_raw[:,0]
DSO_tariff = price_raw[:,1]
TSO_tariff = price_raw[:,2]
electricity_tax = 0.774
price_perhour = []

for i in range(8760):
#    price_perhour.append((1+VAT)*(Spot_price[i] + DSO_tariff[i] + TSO_tariff[i] + electricity_tax))
    price_perhour.append((1+VAT)*(DSO_tariff[i] + TSO_tariff[i] + electricity_tax))
...
# 此处得到 365*24 的 PV_df load_df PC_df PD_df
