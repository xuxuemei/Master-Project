# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 00:17:15 2019
...
PVinput_day_df = pd.DataFrame(PVinput_day)
#load求和
Loadinput_day = []
for i in range(365):
    Loadinput_day.append(load_df.iloc[i,:].sum())
Loadinput_day_df = pd.DataFrame(Loadinput_day)
...
estimator_PV.fit(PVinput_day_df) #聚类
...
estimator_Load.fit(Loadinput_day_df) #聚类
...
X_1 = range(1,len(B[0])+1)
plt.scatter(X_1, B[0],c='w',s=50,marker = 's',edgecolors='r')
X_2 = range(1,len(B[1])+1)
plt.scatter(X_2, B[1],c='w',s=50,marker = 's',edgecolors='y')
X_3 = range(1,len(B[2])+1)
plt.scatter(X_3, B[2],c='w',s=50,marker = 's',edgecolors='b')
X_4 = range(1,len(B[3])+1)
plt.scatter(X_4, B[3],c='w',s=50,marker = 's',edgecolors='g')
plt.xlabel('samples',fontsize=20)
plt.ylabel('PV Sum of a day',fontsize=20)
plt.title('Four clusters of PV production',fontsize=20)
plt.show()

#plot load
plt.figure(figsize=[12,8])
X_11 = range(1,len(C[0])+1)
plt.scatter(X_11, C[0],c='w',s=50,marker = 's',edgecolors='r')
X_22 = range(1,len(C[1])+1)
plt.scatter(X_22, C[1],c='w',s=50,marker = 's',edgecolors='y')
X_33 = range(1,len(C[2])+1)
plt.scatter(X_33, C[2],c='w',s=50,marker = 's',edgecolors='b')
X_44 = range(1,len(C[3])+1)
plt.scatter(X_44, C[3],c='w',s=50,marker = 's',edgecolors='g')
plt.xlabel('Samples',fontsize=20)
plt.ylabel('Load Sum of a day',fontsize=20)
plt.title('Four clusters of Load',fontsize=20)
plt.show()

plt.figure(figsize=[10,8])
cost_list = [cost_load_sum/7.5,cost_opt_sum/7.5,cost_est_sum/7.5] 
plt.bar(range(len(cost_list)), cost_list)  
plt.title('Comparison of cost of electricty for 2018',fontsize=24)
plt.xlabel('Different operations',fontsize=20)
plt.ylabel('Total cost (€)',fontsize=20)
plt.xticks(range(len(cost_list)),['Cost_load','Cost_opt','Cost_est'],fontsize=20)
plt.grid(axis="y")
plt.show()  
